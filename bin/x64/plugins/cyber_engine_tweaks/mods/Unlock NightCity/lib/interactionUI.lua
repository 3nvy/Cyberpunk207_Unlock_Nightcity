...
function ui.init() -- Register needed observers
    ...
    Observe("InteractionUIBase", "OnInteractionData", function(this)
        ui.baseControler = this
    end)
    ...
    Override("InteractionUIBase", "OnInteractionData",
        function(_, value, wrapped) -- Avoid interaction getting overriden by game
            if ui.hubShown then ui.Cleanup() end
            wrapped(value)
        end)
...
function ui.update() -- Run ever frame to avoid unwanted changes
    if ui.hubShown then
        Game.GetBlackboardSystem():Get(GetAllBlackboardDefs().UIInteractions):SetInt(GetAllBlackboardDefs().UIInteractions
            .SelectedIndex, ui.selectedIndex)
    end
    ui.input = false -- Avoid double input
end

function ui.Cleanup(target)
    if not target or not target.GetEntityID or target:GetEntityID().hash ~= ui.entityID then
        ui.hideHub()
        ui.hub = nil
        ui.entityID = nil
    end
